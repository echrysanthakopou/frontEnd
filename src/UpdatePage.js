...
export default function UpdatePage(...pros) {
    ...
    const getAllIssue = () => {
        ...
        axios.post(process.env.REACT_APP_BACKEND_URL + '/Issue', temp1.toString().valueOf(), {headers: {"Content-Type": "text/plain"}}).then(data => {
    ...
    const onSubmit = data => {
        ...
        axios.defaults.headers.post['Access-Control-Allow-Origin'] = '*';
        axios.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';
        axios.post(process.env.REACT_APP_BACKEND_URL + '/updateIssue', data).then(data => {
            setOpen(true);
        });
    }
