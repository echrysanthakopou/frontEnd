...
export default function UpdatePage(...pros) {
    ...
    const getAllIssue = () => {
        ...
        axios.post('http://localhost:8082/Issue', temp1.toString().valueOf(), {headers: {"Content-Type": "text/plain"}}).then(data => {
    ...
    const onSubmit = data => {
        ...
        axios.defaults.headers.post['Access-Control-Allow-Origin'] = '*';
        axios.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';
        axios.post('http://localhost:8082/updateIssue', data).then(data => {
            setOpen(true);
        });
    }
