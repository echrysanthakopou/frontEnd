...
export default function UpdateCommunicationDetails(...pros) {
    ...
    const onSubmit = data => {
        ...
        axios.post('http://localhost:8082/updateDetail', data).then(data => {
            setOpen(true);
        });
    }
    var prosData = pros[0]
    ...
    function getIssues() {
        ...
        var temp1 = prosData.name
        console.log(prosData.name)
        axios.post('http://localhost:8083/getData', temp1.toString().valueOf(), {headers: {"Content-Type": "text/plain"}}).then(resp => {
            setIssues(resp.data);
